...
public:
    int longestCommonSubsequence(string text1, string text2) {
        int len1 = text1.size();        // 行数
        int len2 = text2.size();        // 列数
        if(!len1 || !len2)  return 0;
        /*
        子问题
            f(i,j) = text1[0..i) 和 text2[0..j) 的最长公共子序列
            
            f(0,*) = 0
            f(*,0) = 0
            
            f(i,j) = f(i - 1,j - 1) + 1             if text1[i - 1] = text2[j - 1]
                   = max(f(i - 1,j),f(i,j - 1))     otherwise
        */
        vector<vector<int>> dp(len1 + 1,vector<int>(len2 + 1,0));   // 一次性完成填写边界条件
        /*
            dp[0][0] = 0;
            for(int i = 1 ; i <= m ; ++i)   dp[i][0] = 0;
            for(int i = 1 ; i <= n ; ++j)   dp[0][j] = 0;
        */  
