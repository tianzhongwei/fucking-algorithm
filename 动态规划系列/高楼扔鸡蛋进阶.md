...
public:
    int superEggDrop(int K, int N) {
        ...
        while(getCountfirmFloors(remainTestCount,K) < N) 
            ++remainTestCount;
...
private:
    int getCountfirmFloors(int remainTestCount,int eggsCount) {
        if(remainTestCount == 1 || eggsCount == 1)
            return remainTestCount;
        
        auto p = make_pair(remainTestCount,eggsCount);
        if(M.find(p) != M.end())
            return M[p];

        int upper = getCountfirmFloors(remainTestCount - 1,eggsCount - 1);
        int lower = getCountfirmFloors(remainTestCount - 1,eggsCount);
        M[p] = upper + 1 + lower;
